  ...
  return (
    <div className={cn(
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-5xl">
        ...
        <div className="flex items-center justify-between mb-10">
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-4">
              <Zap className={cn("h-8 w-8", isDark ? "text-blue-500" : "text-primary")} />
              <h1 className="text-3xl font-bold tracking-tight">Simple Reader</h1>
            </div>
            <p className="text-sm text-slate-500 font-medium">Distraction-free quick reference</p>
          </div>
          
          <div className="flex items-center gap-4">
            <Link href="/reader">
              <Button variant="outline" size="sm" className="hidden md:flex gap-3 h-11 rounded-xl border-slate-200 font-bold text-xs uppercase tracking-widest px-6">
                <Layout className="h-5 w-5" /> Switch to Enhanced
              </Button>
            </Link>
            <Button 
              variant="ghost" 
              size="icon" 
              onClick={toggleTheme}
              className={cn("rounded-full h-12 w-12", isDark ? "hover:bg-slate-800 text-slate-400" : "hover:bg-slate-200 text-slate-600")}
            >
        ...
        {/* Controls Bar */}
        <div className={cn(
          "flex flex-wrap items-center gap-4 mb-8 p-4 rounded-xl border transition-colors duration-300",
          isDark ? "bg-slate-900/50 border-slate-800" : "bg-white border-slate-200 shadow-sm"
        )}>
          <Select value={book} onValueChange={setBook}>
            <SelectTrigger className={cn(
              "w-[200px] h-11 text-sm font-bold uppercase tracking-widest focus:ring-blue-500 rounded-lg",
            ...
            <SelectContent className={cn(
              ...
              {BIBLE_BOOKS.map((b) => (
                <SelectItem key={b} value={b} className="text-sm font-bold uppercase tracking-widest">
                  {b}
                </SelectItem>
              ))}
            </SelectContent>
          ...
          <Select value={chapter} onValueChange={setChapter}>
            <SelectTrigger className={cn(
              "w-[120px] h-11 text-sm font-bold uppercase tracking-widest focus:ring-blue-500 rounded-lg",
              isDark ? "bg-slate-800 border-slate-700" : "bg-slate-50 border-slate-200"
            )}>
            ...
            <SelectContent className={cn(
              ...
              {[...Array(150)].map((_, i) => (
                <SelectItem key={i + 1} value={(i + 1).toString()} className="text-sm font-bold uppercase tracking-widest">
                  {i + 1}
                </SelectItem>
              ))}
            </SelectContent>
          ...
          <Select value={version} onValueChange={setVersion}>
            <SelectTrigger className={cn(
              "w-[140px] h-11 text-xs font-bold uppercase tracking-wider focus:ring-blue-500 rounded-lg",
              isDark ? "bg-slate-800 border-slate-700" : "bg-slate-50 border-slate-200"
            )}>
          ...
          <div className="flex items-center gap-3">
            <div className={cn(
              "flex items-center border rounded-lg overflow-hidden h-11 shadow-sm",
              ...
              <Button 
                ...
                className={cn(
                  "border-r rounded-none w-11 h-11",
        ...
        <Card className={cn(
          ...
          <div className="p-12 md:p-20 flex-1 text-center">
            {loading ? (
              <div className="flex flex-col items-center justify-center h-full space-y-6 opacity-40">
                <Loader2 className={cn("h-12 w-12 animate-spin", isDark ? "text-blue-500" : "text-primary")} />
                <p className="text-base font-bold uppercase tracking-[0.2em]">Consulting the Scriptorium...</p>
              </div>
            ) : scripture ? (
              <div className="max-w-3xl mx-auto space-y-14 animate-in fade-in duration-700">
                <div className="space-y-6">
                  <h2 className={cn("text-6xl font-headline font-bold tracking-tight", isDark ? "text-white" : "text-slate-900")}>
                    {scripture.reference}
                  </h2>
                  <p className={cn("text-xs font-bold uppercase tracking-[0.3em]", isDark ? "text-slate-400" : "text-slate-500")}>
                    {SUPPORTED_VERSIONS.find(v => v.id === version)?.name || "OFFICIAL VERSION"}
                  </p>
                  <div className={cn("w-20 h-1.5 mx-auto mt-10 rounded-full", isDark ? "bg-blue-500/20" : "bg-primary/10")} />
                </div>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center space-y-6 opacity-20">
                ...
                <p className="text-xl font-medium">Select a chapter to begin reading</p>
              </div>
            )}
          </div>

          <div className={cn(
            "p-10 pt-6 border-t flex items-center justify-between text-xs font-bold uppercase tracking-widest",
